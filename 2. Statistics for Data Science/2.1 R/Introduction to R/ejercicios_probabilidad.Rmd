...
```{r}
set.seed(1234) #Fijando semilla de números aleatorios
experi<-10000000
...
proba_binom[i+1]<-choose(50,i)*(.3^i)*(.7^(50-i)) }
par(mfrow=c(1,2))
barplot(proba_binom,xlab="P(X=x)")
plot(cumsum(proba_binom),type="l",xlab="P(X<=x")
```
...
```{r}
#1-ppois(0,lambda = 2.55)
#ppois(0,lambda = 2.55) #P(X=0)

1-ppois(3,lambda = 2.55)
...
Repensar el problema, ahora se estudiará cada semáforo como un evento Bernoulli, y buscaremos saber: \
 ...
 _2) en solo un semáforo haya accidentes_ $P(X = 1 | p = 0.70)$. 
$P(X \geq 10 | p=0.7, n=20)$ 
 También, que haya mínimo cinco semáforos con choques. ¿Es posible pensar que haya más de 10 semáforos con choques dada la proporción de *p = 0.70* y *n= 20* ?

```{r}
accidentes<-c(0, 5, 3, 4, 1, 6, 0, 2, 4, 9, 1, 0, 0, 4, 7, 3, 0, 1, 0, 1)
evenbernu<-ifelse(accidentes==0,0,1)
sum(evenbernu)/20 #n=20
```
$P(X=0)$ Considerando la distribución Binomial con n=20,p=0.7
$P(X=1)$ Un semáforo con accidente
Valore esperado de semáforos con accidentes = $n*p$ = $20*0.7 = 14$
...
```{r}
pnorm(602,mediaA,sqrt(varA))-pnorm(598,mediaA,sqrt(varA)) #En este proceso es más probable cumplir con las especificaciones
pnorm(602,mediaB,sqrt(varB))-pnorm(598,mediaB,sqrt(varB))
```
...
```{r}
sismos<-read.csv("P:\\UP\\2025\\2025-2\\SSNMX_catalogo_19000101_20251004 (1).csv")
...
```{r}
hist(sismos2$Magnitud,breaks = seq(1,8.25,0.05))
plot(density(sismos2$Magnitud,na.rm=TRUE))
```
...
$P(X>5.0|Estado =Chiapas)$
$P(X>5.0|Estado =Jalisco)$
$P(X>5.0|Estado =Oaxaca)$
$P(X>5.0|Nacionales)$

```{r}
pnorm(5,media_nac,sd_nac) #Probabilidad de un sismo no-destructor
pnorm(5,media_nac,sd_nac,lower.tail = FALSE) #Probabilidad de un sismo destructor
...
magnitud[order(magnitud$Magnitud*100,decreasing = TRUE),]
```
...
Una compañía fabrica focos con vida media de 500 horas y desviación estándar de 100. Suponga que los tiempos de vida útil de los focos se distribuyen de forma Normal, esto es que los tiempos de vida forman una distribución normal.

- a) Encuentre la probabilidad de que cierta cantidad de focos dure menos de 650 horas. $P(X<650)$ Sea X, la duración de vida de los focos.
...
$ P(650 \leq  X \leq 780) = P(650 < X < 780) $
...
$P(X > ?|\mu = 500, \sigma = 100) = 0.05$

```{r}
pnorm(663,500,100,lower.tail = FALSE)
```

```{r}
qnorm(0.05,500,100,lower.tail = FALSE)
```
```{r}
pnorm(664.4854,500,100)
```

- e) Si se eligen 10,000 focos, ¿cuántos tuvieron un tiempo de vida entre 650 y 780 horas (ambos inclusive)?

```{r}
(pnorm(780,500,100)-pnorm(650,500,100))*10000 (n*p)
```
```{r}
pnorm(780,500,100)-pnorm(650,500,100)
```
...
$P( X_{1} > 780 | \mu = 500, \sigma = 100  )$ $X_{1}$ Es la duración de los focos 
```{r}
peee<-pnorm(780,500,100,lower.tail = FALSE)
```

$ P( X_{2} \geq 3 | p = 0.00255513, n= 1200) $ Con $X_{2}$ el número de focos que fallan en 1200 inspecciones

$P(X_{2}  \geq 3) = 1- P(X_{2} < 3) =1- P(X_{2} \leq 2)$

```{r}
1-pbinom(q=2,prob=peee,size=1200)
```
- g) Si se eligen 20 focos, ¿cuál es la probabilidad de que entre 16 y 19 (ambos inclusive) duren menos de 650 horas?

$P(X_{1} < 650)$ $X_{1}$ es la duración de los focos $X_{1} $ sigue una distribución normal 
$ P(16 \leq X_{2} \leq 19) $ $X_{2}$ sigue una distriución Binomial

$P(15 < X \leq 19) = P(X \leq 19) - P(X \leq 15)  = P(X=16)+P(X=17)+P(X=18)+P(X=19)$ 

```{r}
pee2<-pnorm(650,500,100)
```
```{r}
pbinom(19,prob=pee2,size=20)-pbinom(15,prob=pee2,size=20)
```

```{r}
choose(20,16)*(pee2^16)*((1-pee2)^(20-16))+
choose(20,17)*(pee2^17)*((1-pee2)^(20-17))+
choose(20,18)*(pee2^18)*((1-pee2)^(20-18))+
choose(20,19)*(pee2^19)*((1-pee2)^(20-19))
```
$P(X_{2} = 16) = P(X \leq 16) - P(X \leq 15) $

```{r}
pbinom(q=16,size=20,prob=pee2)-pbinom(q=15,size=20,prob=pee2)
choose(20,16)*(pee2^16)*((1-pee2)^(20-16))
```
...
$ P(X_{1} >650) $ Distribucion normal
$ P(16 \leq X_{2} \leq 19)$

```{r}
pee3<-pnorm(650,500,100,lower.tail = FALSE)
pbinom(19,prob=pee3,size=20)-pbinom(15,prob=pee3,size=20)
20*pee3
```
...
sismos$Magnitud<-as.numeric(sismos$Magnitud)
media_muestra<-rep(NA,10000)
for(i in 1:10000) {
  media_muestra[i]<-mean(sample(sismos$Magnitud,size=20000+i,replace=FALSE),na.rm=TRUE)}
```
